...
int current_trade = TRADE_TYPE_NOT_HAVE;
...
//---- buy conditions
   if(touch_bottom(ma_cur, ma_prev1, ma_prev2)) {
     OrderSend(Symbol(), OP_SELL, NormalizeDouble(AccountFreeMargin()/1000.0,1), Bid, 3, 0, 0, "", MAGICMA, 0, Blue);
     current_trade = TRADE_TYPE_SELL;
     return;
   }
//---- sell conditions
   if(touch_top(ma_cur, ma_prev1, ma_prev2)) {
     OrderSend(Symbol(), OP_BUY, NormalizeDouble(AccountFreeMargin()/1000.0,1), Ask, 3, 0, 0, "", MAGICMA, 0, Red);
     current_trade = TRADE_TYPE_BUY;     
     return;
   }
...
//----
   for(int i = 0;i < OrdersTotal();i++)
	 ...
	 continue;
       ...
       if(OrderType() == OP_BUY)
	 {
	   if(cross_below(ma_cur, ma_prev1) || touch_bottom(ma_cur, ma_prev1, ma_prev2)){
	     OrderClose(OrderTicket(), OrderLots(), Bid, 3, White);
	   }
	   current_trade = TRADE_TYPE_NOT_HAVE;
	   break;
        }
      if(OrderType() == OP_SELL)
        {
	  if(cross_above(ma_cur, ma_prev1) || touch_top(ma_cur, ma_prev1, ma_prev2)){
...
//---- calculate open orders by current symbol
    ...
    if (current_trade == TRADE_TYPE_NOT_HAVE){
      CheckForOpen();
    } else {
      CheckForClose();
    }
