    ...
    def __init__(self, learning_rate=0.001, state_size=15, action_size=3, time_series=32):
        ...
        if IS_TF_STYLE:
            if ENABLE_L2_LEGURALIZER:
                ...
                self.model = tf.keras.Sequential([
                    LSTM(hidden_size, input_shape=(time_series, state_size), return_sequences=True, kernel_regularizer=l1(0.01), activation=None), #recurrent_dropout=0.5),
                    LeakyReLU(0.2),
                    BatchNormalization(),
                    Dropout(0.5),
                    LSTM(hidden_size, return_sequences=False, kernel_regularizer=l1(0.01), activation=None), #recurrent_dropout=0.5),
                    LeakyReLU(0.2),
